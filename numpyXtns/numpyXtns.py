...
def nearest(x):
    ...
    Returns:
        Rounded number (int).
    """
    return x < 0 and int(x-.5) or int(x+.5)


def anearest(x):
    """
    Round float array to nearest integers.

    Args:
        x (float array): input array.

    Returns:
        Nearest integers (int array).
    """
    return where(x < 0, (x-.5).astype(int), (x+.5).astype(int))


def roundSingleDigit(x):
    """
    Round to closest digit.

    Args:
        x (float): input float.

    Return:
        Float rounded to closest single digit.X
    """
    return round(x, -int(floor(log10(x))))  # round to closest digit
